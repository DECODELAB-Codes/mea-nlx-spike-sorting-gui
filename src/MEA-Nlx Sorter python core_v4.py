...
=======
End-to-end GUI for:
  1) Loading MCS/Harvard Apparatus MEA recordings exported as HDF5 (.h5) or Nlx .ncs tetrodes
  2) Simple channel QC (RMS-based) and selection
  3) Spike sorting with MountainSort4 via SpikeInterface (detect_sign=0 by default)
  4) Unit-level reports (waveforms, ISI, ACG, PCA scatter) and quality metrics
  5) Rate-based functional connectivity (sliding-window dot product) with per-unit min–max normalization
  6) Null models via ISI shuffling (pair-specific or pooled) + p-values (two-tailed) + optional FDR
  7) Excel exports for review; large arrays saved as .npy

Notes
=====
- Requires: spikeinterface[full,widgets]>= 0.96, mountainsort4, numpy, scipy, pandas, matplotlib, xlsxwriter, openpyxl, joblib, tqdm, h5py
...
from mpl_toolkits.mplot3d import Axes3D  # noqa: F401 (for 3D PCA)
# Parallel for nulls
from joblib import Parallel, delayed
from tqdm import tqdm

# Excel writer
import xlsxwriter  # noqa: F401 (engine)
...
# Place near the top of your script
def set_tempdir_for_session(path):
    import os, tempfile
    os.makedirs(path, exist_ok=True)
    os.environ['TEMPDIR'] = path
    os.environ['TMP'] = path
...
class NoisyChannelInspector(tk.Toplevel):
    def __init__(self, parent, traces_dict, fs, seconds=60):
        ...
        # --- Add your plots inside scrollable_frame ---
        self.check_vars = {}
        n = len(traces_dict)
        cols = 2
        rows = (n + 1) // 2
...
class MEASettings:
    ...
    # Connectivity
    analysis_offset_s: float = 0.0
    analysis_duration_s: float = 300.0
    bin_size_s: float = 1.0
    window_bins: int = 3

    # Null
    use_pair_specific_null: bool = True
    n_shuffles: int = 1000
    p_two_tailed: bool = True
    fdr_q: Optional[float] = None  # e.g. 0.1, None to skip
    ...
    # Advanced
    save_null_samples: bool = False  # saves full null arrays as .npy (large)
...
# --- Probe helpers (keep near your other utils) ---
def attach_probe_to_recording(recording, locations_um, channel_ids,
    ...
    # Here we assume the contact order == recording channel order you passed in.
    ...
    # Attach probe (preferred API on your SI 0.103)
...
def ensure_probe_on(recording, reference_rec):
    ...
    # Rebuild a probe aligned to *this* recording’s channel order
    from probeinterface import Probe
    import numpy as np
...
def preprocess_recording(recording, settings: MEASettings):
    ...
    if getattr(settings, "use_notch_60hz", True):
        rec = spre.notch_filter(rec, freq=60.0, q=30)
    if settings.use_cmr:
...
def print_si_slicing_diagnostics(recording=None):
    ...
    # 6) any other oddball method name
    try:
        if hasattr(recording, "select_channels"):
...
def _filter_sorter_kwargs(sorter_name: str, params: dict):
    """
    Keep only keys accepted by this sorter in the current SI version.
    If we can't fetch defaults, return as-is.
    """
    try:
        import spikeinterface.sorters as ss
        defaults = ss.get_default_params(sorter_name)  # robust across SI versions
...
def run_mountainsort4(recording, settings: MEASettings, output_folder: str):
    """MountainSort4: no re-filter (done upstream), DO whiten inside MS4."""
    import spikeinterface.sorters as ss

    sorting = ss.run_sorter(
        sorter_name="mountainsort4",
        recording=recording,
        folder=output_folder,              # SI 0.103 uses 'folder'
        remove_existing_folder=True,
        verbose=True,

        # MS4 params from your UI
        ...
        detect_interval=getattr(settings, "detect_interval", 10),  # <- NEW

        # Preproc policy
        filter=False,                      # bandpass/CMR done upstream
...
def run_kilosort4(recording, settings: MEASettings, output_folder: str):
    ...
    # Filter to only keys accepted by this SI version
...
def run_spykingcircus2(recording, settings: MEASettings, output_folder: str):
    ...
    params = dict(
        filter=False,                 # avoid double bandpass (you did it upstream)
        whiten=True,
        num_workers=getattr(settings, "n_jobs", 8),
        apply_preprocessing=False,    # UI JSON can flip this to True
    )
    ...
    if js:
        try:
            user = json.loads(js)
            if isinstance(user, dict):
                if "apply_preprocessing" in user:
                    params.pop("apply_preprocessing", None)
                params.update(user)
        except Exception:
            pass
...
def export_unit_spike_times(sorting, fs: float, out_xlsx: str):
    ...
    with pd.ExcelWriter(out_xlsx, engine='xlsxwriter') as writer:
        ...
        if max_len <= MAX_ROWS:
            # Single-sheet case (your preferred layout)
            data = {}
...
def _compute_waveform_features(w, fs_hz, flip_to_negative_first=True):
    ...
    left_idx  = slice(max(0, i_tr - 200), i_tr + 1)                          # generous window
...
def make_unit_report_figures(
    ...
    if templates is not None:
        for uid in unit_ids:
            try:
                T = templates[uid_to_idx[uid], :, :]  # (samples, channels) or (samples,) in rare cases
                if T.ndim == 1:
                    T = T[:, None]
                ptp = np.ptp(T, axis=0)
    ...
    for uid in unit_ids:
        ...
        if len(isis) > 0:
            ax.hist(isis, bins='auto', alpha=0.9)
        ax.set_title(f"Unit {uid} — ISI histogram")
        ax.set_xlabel("ISI (s)")
        ax.set_ylabel("Count")
        fig.tight_layout()
        fig.savefig(os.path.join(output_dir, f"unit_{uid:03d}_isi_hist.png"), dpi=200)
        plt.close(fig)
    ...
    if stats_rows:
        ...
        # Build the list of new/feature columns in the order you prefer
...
def bin_unit_spike_times(unit_spikes_s: Dict[int, np.ndarray], start_s: float, duration_s: float, bin_s: float) -> Tuple[np.ndarray, List[int]]:
    """Return (rates_matrix, unit_order). rates has shape (n_units, n_bins) in spikes/s."""
    units = sorted(unit_spikes_s.keys())
    n_bins = int(np.floor(duration_s / bin_s))
    edges = np.linspace(start_s, start_s + duration_s, n_bins + 1)
    rates = np.zeros((len(units), n_bins), dtype=np.float32)
    for i, uid in enumerate(units):
        st = unit_spikes_s[uid]
        # Restrict to [start, start+duration]
        st = st[(st >= start_s) & (st <= start_s + duration_s)]
        if st.size:
            counts, _ = np.histogram(st, bins=edges)
            rates[i, :] = counts.astype(np.float32) / bin_s
    return rates, units


def minmax_normalize_per_unit(rates: np.ndarray) -> np.ndarray:
    z = rates.astype(np.float32, copy=True)
    mins = z.min(axis=1, keepdims=True)
    maxs = z.max(axis=1, keepdims=True)
    spans = np.maximum(maxs - mins, 1e-9)
    z = (z - mins) / spans
    return z


def compute_mean_adjacency_from_rates(z: np.ndarray, window_bins: int) -> np.ndarray:
    """Compute mean adjacency via sliding dot-product. z: (n_units, n_bins). Returns (n_units, n_units)."""
    n_units, n_bins = z.shape
    if window_bins <= 0 or window_bins > n_bins:
        raise ValueError("window_bins must be in [1, n_bins]")
    n_windows = n_bins - window_bins + 1
    acc = np.zeros((n_units, n_units), dtype=np.float64)
    for start in range(n_windows):
        seg = z[:, start:start + window_bins]  # (n_units, window_bins)
        acc += (seg @ seg.T) / float(window_bins)
    mean_adj = acc / float(n_windows)
    return mean_adj


def upper_triangle_table(adj: np.ndarray, units: List[int], meta: Dict[str, str]) -> pd.DataFrame:
    pairs = []
    n = len(units)
    for i in range(n):
        for j in range(i + 1, n):
            pairs.append((units[i], units[j], adj[i, j]))
    df = pd.DataFrame(pairs, columns=['Unit_i', 'Unit_j', 'Mean Edge Weight'])
    for k, v in meta.items():
        df[k] = v
    return df


def isi_shuffle(spike_times: np.ndarray, duration_s: float, rng: np.random.Generator) -> np.ndarray:
    ...
    # Start at original first spike for parity; truncate to window
    start = spike_times.min()
    rebuilt = [start]
    for d in shuffled:
        nxt = rebuilt[-1] + d
        if nxt >= start + duration_s:
            break
        rebuilt.append(nxt)
    return np.array(rebuilt, dtype=float)


def build_pair_specific_null(unit_spikes_s: Dict[int, np.ndarray],
                             start_s: float,
                             duration_s: float,
                             bin_s: float,
                             window_bins: int,
    ...
    """Return dict[(i,j)] = null edge array (length n_shuffles)."""
    ...
    def one_pair(i, j):
        ...
        s_i = unit_spikes_s[ui]
        s_j = unit_spikes_s[uj]
        rng_pair = np.random.default_rng(seed + 1000003 * i + 9176 * j)  # NEW
        vals = np.zeros(n_shuffles, dtype=np.float32)
        for k in range(n_shuffles):
            si_sh = isi_shuffle(s_i, duration_s, rng_pair)
            sj_sh = isi_shuffle(s_j, duration_s, rng_pair)
            # Bin
            c_i, _ = np.histogram(si_sh[(si_sh >= start_s) & (si_sh <= start_s + duration_s)], bins=edges)
            c_j, _ = np.histogram(sj_sh[(sj_sh >= start_s) & (sj_sh <= start_s + duration_s)], bins=edges)
            r_i = c_i.astype(np.float32) / bin_s
            r_j = c_j.astype(np.float32) / bin_s
            # Normalize per unit
            zi = minmax_normalize_per_unit(r_i[None, :])[0]
            zj = minmax_normalize_per_unit(r_j[None, :])[0]
            # Edge via sliding dot mean
            n_w = len(zi) - window_bins + 1
            acc = 0.0
            for s in range(n_w):
                acc += float(np.dot(zi[s:s+window_bins], zj[s:s+window_bins]) / float(window_bins))
            vals[k] = acc / float(n_w)
        return (ui, uj), vals

    results = {}
    # Parallel over pairs (light parallelism to avoid oversubscription)
    pairs = [(i, j) for i in range(len(units)) for j in range(i + 1, len(units))]
    n_jobs = min( max(1, os.cpu_count() // 2), len(pairs) )
    results_list = Parallel(n_jobs=n_jobs)(
        delayed(one_pair)(i, j) for i, j in pairs
    )
    for (ui, uj), arr in results_list:
        results[(ui, uj)] = arr
    return results


def build_pooled_null(unit_spikes_s: Dict[int, np.ndarray],
                      start_s: float,
                      duration_s: float,
                      bin_s: float,
                      window_bins: int,
    ...
    """Return a single pooled null distribution of mean-edge values across all pairs and shuffles."""
    rng = np.random.default_rng(seed)
    units = sorted(unit_spikes_s.keys())
    # Precompute edges
    n_bins = int(np.floor(duration_s / bin_s))
    ...
    pooled = []
    for k in tqdm(range(n_shuffles), desc="Pooled null shuffles"):
        # Shuffle each unit independently
        sh_spikes = {}
        for u in units:
            sh_spikes[u] = isi_shuffle(unit_spikes_s[u], duration_s, rng)
        # Bin all
        rates = np.zeros((len(units), n_bins), dtype=np.float32)
        for i, u in enumerate(units):
            ...
            rates[i, :] = c.astype(np.float32) / bin_s
        z = minmax_normalize_per_unit(rates)
        adj = compute_mean_adjacency_from_rates(z, window_bins)
        n = len(units)
        for i in range(n):
            for j in range(i + 1, n):
                pooled.append(adj[i, j])
    return np.array(pooled, dtype=np.float32)

def percentile_p_two_tailed(null_vals: np.ndarray, actual: float) -> float:
    if null_vals.size == 0:
        return 1.0
    lt = np.mean(null_vals < actual)
    eq = np.mean(null_vals == actual)
    rank = lt + 0.5 * eq
    p = 2.0 * min(rank, 1.0 - rank)
    return float(min(max(p, 0.0), 1.0))

def percentile_p_one_tailed(null_vals: np.ndarray, actual: float, tail: str = "greater") -> float:
    """Empirical one-tailed p-value from null distribution."""
    if null_vals.size == 0:
        return 1.0
    if tail == "greater":
        return float(np.mean(null_vals >= actual))
    elif tail == "less":
        return float(np.mean(null_vals <= actual))
    else:
...
class MEAGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("MEA Spike Sorting & Connectivity (SpikeInterface)")
        self.geometry("1120x780")
        self.settings = MEASettings()
        self.recording = None
        self.raw_recording = None        # NEW: keep a raw handle for KS4 routing
    ...
    def _build_tab_conn(self):
        f = self.tab_conn
        r = 0
        conn_frame = ttk.LabelFrame(f, text="Connectivity settings")
        conn_frame.grid(row=r, column=0, columnspan=4, sticky='we', pady=6)
        ttk.Label(conn_frame, text="Start offset (s)").grid(row=0, column=0, sticky='e')
        self.var_off = tk.DoubleVar(value=self.settings.analysis_offset_s)
        ttk.Entry(conn_frame, textvariable=self.var_off, width=8).grid(row=0, column=1, sticky='w')
        ttk.Label(conn_frame, text="Duration (s)").grid(row=0, column=2, sticky='e')
        self.var_dur = tk.DoubleVar(value=self.settings.analysis_duration_s)
        ttk.Entry(conn_frame, textvariable=self.var_dur, width=8).grid(row=0, column=3, sticky='w')
        ttk.Label(conn_frame, text="Bin size (s)").grid(row=0, column=4, sticky='e')
        self.var_bin = tk.DoubleVar(value=self.settings.bin_size_s)
        ttk.Entry(conn_frame, textvariable=self.var_bin, width=8).grid(row=0, column=5, sticky='w')
        ttk.Label(conn_frame, text="Window (bins)").grid(row=0, column=6, sticky='e')
        self.var_win = tk.IntVar(value=self.settings.window_bins)
        ttk.Entry(conn_frame, textvariable=self.var_win, width=8).grid(row=0, column=7, sticky='w')
        ttk.Button(conn_frame, text="Compute mean edges", command=lambda: self._run_bg(self.compute_mean_edges)).grid(row=0, column=8, padx=10)

    def compute_mean_edges(self):
        try:
            if self.sorting is None or self.waveforms is None:
                messagebox.showwarning("Missing", "Run sorting first")
                return
            # Build unit spike dict in seconds
            fs = float(self.waveforms.recording.get_sampling_frequency())
            self.unit_spikes_s = {}
            for uid in self.sorting.get_unit_ids():
                st = self.sorting.get_unit_spike_train(unit_id=uid)
                self.unit_spikes_s[uid] = (st / fs).astype(float)            
            # ADD THIS GUARD:
            if len(self.unit_spikes_s) == 0:
                messagebox.showwarning("Connectivity", "No units found — cannot compute connectivity.")
                return
            if len(self.unit_spikes_s) == 1:
                messagebox.showwarning("Connectivity", "Need at least 2 units to compute connectivity.")
                return
            
            start = float(self.var_off.get())
            duration = float(self.var_dur.get())
            bin_s = float(self.var_bin.get())
            win = int(self.var_win.get())
            
            # recording length check
            fs = float(self.waveforms.recording.get_sampling_frequency())
            total_s = self.waveforms.recording.get_num_frames() / fs
            if start < 0 or start >= total_s:
                messagebox.showwarning("Connectivity", f"Start {start:.1f}s is outside recording ({total_s:.1f}s).")
                return
            if start + duration > total_s:
                self.log(f"Warning: end {start+duration:.1f}s exceeds recording ({total_s:.1f}s). Truncating.")
                duration = max(0.0, total_s - start)
            
            # bins/window sanity
            n_bins = int(math.floor(duration / bin_s))
            if n_bins <= 0:
                messagebox.showwarning("Connectivity", "Duration/bin size yields zero bins. Adjust settings.")
                return
            if win > n_bins:
                messagebox.showwarning("Connectivity", f"Window ({win}) > number of bins ({n_bins}). Reduce window or increase duration / reduce bin size.")
                return
            
            rates, units = bin_unit_spike_times(self.unit_spikes_s, start, duration, bin_s)
            z = minmax_normalize_per_unit(rates)
            adj = compute_mean_adjacency_from_rates(z, win)

            meta = {
                'Animal': self.settings.animal,
                'UniqueID': self.settings.unique_id,
                'Group': self.settings.group,
                'Cohort': self.settings.cohort,
            }
            df = upper_triangle_table(adj, units, meta)
            out = os.path.join(self.var_out.get() or os.getcwd(), "mean_edge_weights.xlsx")
            df.to_excel(out, index=False)
            self.log(f"Mean edge weights saved: {out}")
        except Exception as e:
            self.log(f"Connectivity error: {e}")
            traceback.print_exc()
            messagebox.showerror("Connectivity error", str(e))

    # --- Tab 4: Null & P-values
    def _build_tab_null(self):
        f = self.tab_null
        r = 0
        null_frame = ttk.LabelFrame(f, text="Null settings (ISI shuffling on units)")
        null_frame.grid(row=r, column=0, columnspan=4, sticky='we', pady=6)
    
        ttk.Label(null_frame, text="# shuffles").grid(row=0, column=0, sticky='e')
        self.var_nshuf = tk.IntVar(value=self.settings.n_shuffles)
        ttk.Entry(null_frame, textvariable=self.var_nshuf, width=8).grid(row=0, column=1, sticky='w')
    
        self.var_pair = tk.BooleanVar(value=self.settings.use_pair_specific_null)
        ttk.Checkbutton(null_frame, text="Pair-specific null (recommended)", variable=self.var_pair).grid(row=0, column=2, padx=10)
    
        ttk.Label(null_frame, text="FDR q (blank=off)").grid(row=0, column=3, sticky='e')
        self.var_fdr = tk.StringVar(value="")
        ttk.Entry(null_frame, textvariable=self.var_fdr, width=8).grid(row=0, column=4, sticky='w')
    
        # NEW: tail selector
        ttk.Label(null_frame, text="P-value tail").grid(row=0, column=5, sticky='e')
        self.var_tail = tk.StringVar(value="two-tailed" if self.settings.p_two_tailed else "one-tailed")
        ttk.Combobox(null_frame,
                     textvariable=self.var_tail,
                     values=("two-tailed", "one-tailed"),
                     width=12,
                     state="readonly").grid(row=0, column=6, sticky='w')
    
        ttk.Button(null_frame, text="Run null & p-values",
                   command=lambda: self._run_bg(self.run_null_and_p)).grid(row=0, column=7, padx=10)

    def run_null_and_p(self):
        try:
            if not self.unit_spikes_s:
                messagebox.showwarning("Missing", "Compute mean edges first (Tab 3)")
                return
            ...
            # IMPORTANT: take units from binning so indices match actual_adj
            rates, units = bin_unit_spike_times(self.unit_spikes_s, start, duration, bin_s)
            z = minmax_normalize_per_unit(rates)
            actual_adj = compute_mean_adjacency_from_rates(z, win)
    
            # C1: sync tail choice from UI (default to settings if widget missing)
            if hasattr(self, "var_tail"):
                self.settings.p_two_tailed = (self.var_tail.get() == "two-tailed")
    
            outdir = self.var_out.get() or os.getcwd()
    
            if self.var_pair.get():
                self.log("Building pair-specific nulls…")
                null_dict = build_pair_specific_null(self.unit_spikes_s, start, duration, bin_s, win, nsh, seed)
                if self.settings.save_null_samples:  # only save if requested
                    np.save(os.path.join(outdir, "null_pair_specific.npy"), null_dict, allow_pickle=True)
    
                # Compute p-values per pair
                rows = []
                for i, ui in enumerate(units):
                    for j, uj in enumerate(units):
                        if j <= i:
                            continue
                        null_vals = null_dict.get((ui, uj), np.array([]))
                        actual = float(actual_adj[i, j])
    
                        # C2: Two-tailed vs one-tailed
                        if self.settings.p_two_tailed:
                            p = percentile_p_two_tailed(null_vals, actual)
                        else:
                            # Edges are non-negative with min–max + dot-product; test "greater" by default
                            p = percentile_p_one_tailed(null_vals, actual, tail="greater")
    
                        rows.append(dict(Unit_i=ui, Unit_j=uj, Actual=actual, P_Value=p))
                res_df = pd.DataFrame(rows)
    
            else:
                self.log("Building pooled null…")
                pooled = build_pooled_null(self.unit_spikes_s, start, duration, bin_s, win, nsh, seed)
                if self.settings.save_null_samples:
                    np.save(os.path.join(outdir, "null_pooled.npy"), pooled)
                ...
                for i in range(n):
                    for j in range(i + 1, n):
                        ...
                        # C2: Two-tailed vs one-tailed
                        if self.settings.p_two_tailed:
                            p = percentile_p_two_tailed(pooled, actual)
                        else:
                            p = percentile_p_one_tailed(pooled, actual, tail="greater")
    
                        rows.append(dict(Unit_i=units[i], Unit_j=units[j], Actual=actual, P_Value=p))
                res_df = pd.DataFrame(rows)
    
            # FDR if requested
            fdr_txt = self.var_fdr.get().strip()
            if fdr_txt:
                try:
                    q = float(fdr_txt)
                    mask = bh_fdr(res_df['P_Value'].values, q)
                    res_df['FDR_q'] = q
                    res_df['Significant'] = mask
                except Exception:
                    self.log("Invalid FDR q — skipping")
    
            out = os.path.join(outdir, "significance_results_with_p_values.xlsx")
            res_df.to_excel(out, index=False)
            self.log(f"Significance saved: {out}")
    
        except Exception as e:
            self.log(f"Null error: {e}")
            traceback.print_exc()
            messagebox.showerror("Null error", str(e))
    
    # --- Tab 5: Export/Batch (skeleton)
    def _build_tab_export(self):
        f = self.tab_export
        r = 0
        ttk.Label(f, text="Metadata (optional)").grid(row=r, column=0, sticky='w')
        r += 1
        meta = ttk.Frame(f)
        meta.grid(row=r, column=0, sticky='w')
        ttk.Label(meta, text="Animal").grid(row=0, column=0)
        self.var_animal = tk.StringVar(value=self.settings.animal)
        ttk.Entry(meta, textvariable=self.var_animal, width=12).grid(row=0, column=1)
        ttk.Label(meta, text="UniqueID").grid(row=0, column=2)
        self.var_uid = tk.StringVar(value=self.settings.unique_id)
        ttk.Entry(meta, textvariable=self.var_uid, width=12).grid(row=0, column=3)
        ttk.Label(meta, text="Group").grid(row=0, column=4)
        self.var_group = tk.StringVar(value=self.settings.group)
        ttk.Entry(meta, textvariable=self.var_group, width=12).grid(row=0, column=5)
        ttk.Label(meta, text="Cohort").grid(row=0, column=6)
        self.var_cohort = tk.StringVar(value=self.settings.cohort)
        ttk.Entry(meta, textvariable=self.var_cohort, width=12).grid(row=0, column=7)
        ttk.Button(meta, text="Save metadata", command=self.save_metadata).grid(row=0, column=8, padx=10)

        r += 1
        ttk.Button(f, text="Open output folder", command=self.open_outdir).grid(row=r, column=0, sticky='w', pady=10)

    def save_metadata(self):
        self.settings.animal = self.var_animal.get()
        self.settings.unique_id = self.var_uid.get()
        self.settings.group = self.var_group.get()
        self.settings.cohort = self.var_cohort.get()
        d = asdict(self.settings)
        out = os.path.join(self.var_out.get() or os.getcwd(), "analysis_settings.json")
        with open(out, 'w') as f:
            json.dump(d, f, indent=2)
        self.log(f"Saved settings: {out}")
